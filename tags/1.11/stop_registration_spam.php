...
<?php
	...
	// strip slashes if magic quote is on (leave escaping /s in database, but make text nice for user
	if (get_magic_quotes_gpc() == 1){
		$opt1_val = stripslashes($opt1_val);
		$opt2_val = stripslashes($opt2_val);
		$opt3_val = stripslashes($opt3_val);
	};
...
// strip slashes that may have been inserted by magic quotes
if (get_magic_quotes_gpc() == 1){
	$tom_custom_question_q = stripslashes($tom_custom_question_q);
	$tom_custom_question_a = stripslashes($tom_custom_question_a);
	$tom_custom_question_z = stripslashes($tom_custom_question_z);
};
...
function tom_custom_answer_f($login,$email,$errors){
 global $tom_custom_question_a,$tom_custom_question_z;
   if (strtolower($_POST['tomcustomq']) != $tom_custom_question_a){
